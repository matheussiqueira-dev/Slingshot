...
const gestureBarEl = document.querySelector("#gestureBar span");
...
let hands = null;
let handsLoopId = null;
let handsBusy = false;
let lastHandsFrame = 0;
...
function initGame() {
  ...
  updateScore();
  updateLevel();
  updateStatus("Pronto para jogar. Use a pinça ou o mouse.");
}
...
function updateAssistant() {
  if (!state.hintsEnabled) {
    ...
    state.assistant.message = "Copiloto desativado. Você está no controle.";
  ...
  if (suggestion) {
    ...
    state.assistant.message = `Sugestao: mire no espaco destacado para formar ${suggestion.size} bolhas.`;
  } else {
    state.assistant.target = null;
    state.assistant.message = "Nao ha combinacao direta. Tente abrir caminho para a proxima cor.";
  }
...
function updateAimFromDrag(currentX, currentY) {
  ...
  if (dist > 4) {
    ...
    gestureBarEl.style.width = `${Math.round(power * 100)}%`;
  }
}

function updateAimFromGesture(handPos, isPinching) {
  const now = performance.now();
  state.gesture.lastSeen = now;
  const { range, neutral, config } = state.gesture;

  if (isPinching) {
    if (!state.gesture.pinching) {
      state.gesture.pinching = true;
      state.gesture.pinchStart = { x: handPos.x, y: handPos.y };
      state.aim.active = true;
    }

    const pullX = state.gesture.pinchStart.x - handPos.x;
    const pullY = state.gesture.pinchStart.y - handPos.y;
    const scaledX = pullX * state.width;
    const scaledY = pullY * state.height;
    const dist = Math.hypot(scaledX, scaledY);
    const power = clamp(dist / (state.width * config.pullScale), 0, 1);

    if (dist > 6) {
      const angle = Math.atan2(scaledY, scaledX);
      state.aim.angle = lerpAngle(state.aim.angle, clamp(angle, state.config.minAngle, state.config.maxAngle), 0.25);
      state.aim.power = lerp(state.aim.power, power, 0.2);
    }

    state.gesture.pullPower = power;
    gestureBarEl.style.width = `${Math.round(power * 100)}%`;
    gestureStatusEl.textContent = "Pinça detectada";
  } else {
    if (state.gesture.pinching) {
      state.gesture.pinching = false;
      if (state.aim.power > 0.05) {
        shootBubble(state.aim.angle, state.aim.power);
      }
      state.aim.active = false;
      state.gesture.pullPower = 0;
      gestureBarEl.style.width = "0%";
    }

    const safeRangeX = range.x || 0.18;
    const safeRangeY = range.y || 0.22;
    let dx = (handPos.x - neutral.x) / safeRangeX;
    let dy = (handPos.y - neutral.y) / safeRangeY;
    dx = applyDeadzone(clamp(dx, -1, 1), config.deadzone);
    dy = applyDeadzone(clamp(dy, -1, 1), config.deadzone);
    const targetX = state.shooter.x + dx * state.width * config.aimScaleX;
    const targetY = state.shooter.y + dy * state.height * config.aimScaleY;
    const angle = Math.atan2(targetY - state.shooter.y, targetX - state.shooter.x);
    state.aim.angle = lerpAngle(state.aim.angle, clamp(angle, state.config.minAngle, state.config.maxAngle), 0.12);
    state.aim.active = true;

    gestureStatusEl.textContent = "Mao detectada";
  }
}

function handlePointerDown(event) {
  if (state.input.mode === "webcam" && performance.now() - state.gesture.lastSeen < 1200) return;
...
function handlePointerMove(event) {
  ...
  state.input.lastPointerTime = performance.now();
...
function onHandResults(results) {
  if (!results.multiHandLandmarks || results.multiHandLandmarks.length === 0) {
    if (state.gesture.calibrating && performance.now() - state.gesture.calibrationStart > 1600) {
      finishCalibration();
    }
    if (performance.now() - state.gesture.lastSeen > 1200) {
      state.gesture.available = false;
      gestureStatusEl.textContent = "Aguardando mao";
      gestureBarEl.style.width = "0%";
    }
    return;
  }
  ...
  state.gesture.smooth.x = lerp(state.gesture.smooth.x, handX, 0.35);
  state.gesture.smooth.y = lerp(state.gesture.smooth.y, handY, 0.35);

  const thumb = landmarks[4];
  const index = landmarks[8];
  const palm = landmarks[0];
  const palmScale = Math.hypot(landmarks[9].x - palm.x, landmarks[9].y - palm.y);
  const pinchDist = Math.hypot(thumb.x - index.x, thumb.y - index.y);
  const pinchRatio = pinchDist / (palmScale + 0.0001);
  const isPinching =
    pinchRatio < state.gesture.config.pinchThreshold || pinchDist < state.gesture.config.pinchDistance;

  state.gesture.available = true;
  state.input.mode = "webcam";

  if (state.gesture.calibrating) {
    state.gesture.calibrationSamples.push({
      x: state.gesture.smooth.x,
      y: state.gesture.smooth.y,
    });
    if (performance.now() - state.gesture.calibrationStart > 1600) {
      finishCalibration();
    }
    return;
  }

  updateAimFromGesture({ x: state.gesture.smooth.x, y: state.gesture.smooth.y }, isPinching);
}

function finishCalibration() {
  const samples = state.gesture.calibrationSamples;
  state.gesture.calibrating = false;
  state.gesture.pinching = false;
  ...
  if (!samples.length) {
    updateStatus("Calibracao falhou. Tente novamente.");
    gestureStatusEl.textContent = "Calibracao incompleta";
    return;
  }
  ...
  const distancesX = samples.map((s) => Math.abs(s.x - avgX)).sort((a, b) => a - b);
  const distancesY = samples.map((s) => Math.abs(s.y - avgY)).sort((a, b) => a - b);

  const idx = Math.floor(distancesX.length * 0.9);
  const rangeX = clamp(distancesX[idx] || 0.12, 0.08, 0.3);
  const rangeY = clamp(distancesY[idx] || 0.14, 0.08, 0.32);

  state.gesture.neutral = { x: avgX, y: avgY };
  state.gesture.range = { x: rangeX, y: rangeY };
  state.gesture.calibrationSamples = [];

  updateStatus("Calibracao concluida. Teste o gesto.");
  gestureStatusEl.textContent = "Calibrado";
}
...
function pickBrioDevice(devices) {
  if (!devices.length) return null;
  const exact = devices.find((device) => /brio\s*305/i.test(device.label));
  if (exact) return exact;
  const generic = devices.find((device) => /brio/i.test(device.label));
  return generic || null;
}
...
async function requestCameraStream(deviceId) {
  ...
  const constraints = deviceId
    ? { video: { ...baseConstraints, deviceId: { exact: deviceId } }, audio: false }
...
function startHandsLoop() {
  if (handsLoopId) {
    cancelAnimationFrame(handsLoopId);
    handsLoopId = null;
  }
...
async function startCamera() {
  if (!window.Hands) {
    updateStatus("MediaPipe nao carregou. Use o mouse.");
    return;
  }

  try {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      updateStatus("Navegador sem suporte a camera. Use o mouse.");
      return;
    }
    ...
    let brioDevice = pickBrioDevice(devices);

    if (!brioDevice || !brioDevice.label) {
      ...
      brioDevice = pickBrioDevice(devices);
    }

    let stream = null;
    if (brioDevice && brioDevice.deviceId) {
      try {
        stream = await requestCameraStream(brioDevice.deviceId);
      } catch (error) {
        stream = await requestCameraStream(null);
      }
    } else {
      stream = await requestCameraStream(null);
    }
    ...
    if (brioDevice && brioDevice.label) {
      updateStatus(`Camera ativa: ${brioDevice.label}. Clique em Calibrar para ajustar.`);
    } else {
      updateStatus("Camera ativa (BRIO 305 nao encontrada). Clique em Calibrar para ajustar.");
    }
    gestureStatusEl.textContent = "Aguardando mao";
    toggleCameraBtn.textContent = "Câmera: On";
  } catch (error) {
    updateStatus("Nao foi possivel acessar a camera. Use o mouse.");
    gestureStatusEl.textContent = "Camera indisponivel";
  }
}
...
function stopCamera() {
  ...
  cameraStream = null;
  updateStatus("Camera desligada. Use o mouse.");
  gestureStatusEl.textContent = "Camera desligada";
  gestureBarEl.style.width = "0%";
  toggleCameraBtn.textContent = "Câmera: Off";
...
function toggleCamera() {
  if (cameraStream || state.cameraActive) {
    stopCamera();
  } else {
    startCamera();
  }
...
function toggleHints() {
  ...
  toggleHintsBtn.textContent = state.hintsEnabled ? "Copiloto: On" : "Copiloto: Off";
  state.needsHint = true;
}
...
function startCalibration() {
  ...
  if (state.gesture.calibrating) return;
  state.gesture.calibrating = true;
  state.gesture.calibrationStart = performance.now();
  state.gesture.calibrationSamples = [];
  state.gesture.pinching = false;
  state.gesture.pinchStart = null;
  state.aim.active = false;
  gestureBarEl.style.width = "0%";
  gestureStatusEl.textContent = "Calibrando...";
  updateStatus("Calibrando: mao aberta e movimentos leves por 2s.");
}
